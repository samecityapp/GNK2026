...
export async function RelatedHotels({ location }: { location: string }) {
    ...
    <div className="mt-8 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border border-blue-100 overflow-hidden">
      <div className="p-6 border-b border-blue-100 flex items-center gap-3">
        ...
        <h3 className="text-xl font-semibold text-gray-900 tracking-tight">
          {location} Bölgesindeki Popüler Oteller
        </h3>
      </div>
          ...
          >
            ...
            <div className="flex-1 flex flex-col justify-center min-w-0">
              ...
              <div className="flex items-center gap-1 text-xs text-gray-600 mb-1">
                ...
                <span className="line-clamp-1">{getLocalizedText(hotel.location)}</span>
              </div>
              {hotel.gnkScore && (
                <div className="flex items-center gap-1 text-xs font-medium text-blue-600">
                  <Star className="w-3 h-3 fill-blue-600" />
                  <span>{hotel.gnkScore}/10 GNK Puanı</span>
                </div>
              )}
            </div>
      ...
      <div className="px-6 pb-6">
        ...
        >
          {location} Bölgesindeki Tüm Otelleri Görüntüle
        </Link>
      </div>
    </div>
  );
